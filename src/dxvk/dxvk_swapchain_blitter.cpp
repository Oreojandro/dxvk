...
namespace dxvk {
  ...
  VkPipeline DxvkSwapchainBlitter::createPipeline(
    ...
    specConstants.gammaBound = key.needsGamma;
    ...
    // and image properties match and no gamma curve is applied.
    if (specConstants.srcSpace == specConstants.dstSpace && (specConstants.srcSpace != VK_COLOR_SPACE_SRGB_NONLINEAR_KHR
      || (specConstants.srcIsSrgb == specConstants.dstIsSrgb && !specConstants.gammaBound))) {
